...
Vector3 MatrixFunctions::Multiply(const Vector3 v, const Matrix4x4 m) {
	...
	tmp.x = tmp2.x * m.m[0][0] + tmp2.y * m.m[1][0] + tmp2.z * m.m[2][0] + tmp2.w * m.m[3][0];
	tmp.y = tmp2.x * m.m[0][1] + tmp2.y * m.m[1][1] + tmp2.z * m.m[2][1] + tmp2.w * m.m[3][1];
	tmp.z = tmp2.x * m.m[0][2] + tmp2.y * m.m[1][2] + tmp2.z * m.m[2][2] + tmp2.w * m.m[3][2];
...
Matrix4x4 MatrixFunctions::Inverse(const Matrix4x4& m) {
	Matrix4x4 tmp;
	float A_;
	A_ = m.m[0][0] * m.m[1][1] * m.m[2][2] * m.m[3][3] +
